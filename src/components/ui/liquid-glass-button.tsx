  ...
  return (
    <>
      ...
      >
        ...
        <span className="pointer-events-none z-10 block">
          {children}
        </span>
        <GlassFilter />
      </Comp>
...
const ShineEffect = ({ isPressed }: { isPressed: boolean }) => {
  return (
    <span
      className={cn(
        "pointer-events-none absolute inset-0 z-20 overflow-hidden transition-opacity duration-300 block",
        isPressed ? "opacity-20" : "opacity-0",
      )}
    >
      <span className="absolute inset-0 rounded-md bg-gradient-to-r from-transparent via-neutral-100 to-transparent block" />
    </span>
  );
};
    ...
    <div className={variants.wrapper} style={variants.wrapperStyle}>
      ...
      >
        ...
        {isHovered && !isPressed && !isTouchDevice && (
          <span className="pointer-events-none absolute inset-0 bg-gradient-to-t rounded-lg from-transparent to-white/5 block" />
        )}
      </button>
    </div>
  );
